...
| Quality gates 1–6 | 264 tests pass in ~4s; `pyyaml` is still the only third-party runtime import; no `shell=True` (grep-verified); every module imports with no tools installed; a missing tool yields `unsupported`, never a fabricated success |
