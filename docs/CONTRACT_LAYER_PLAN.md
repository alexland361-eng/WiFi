...
| M4: each subsystem produces/consumes only its contracts; verification never invokes a tool | `test_evidence_engine.py` (28), `test_verification_engine.py` (29), `test_world_state.py` (34), `test_policy.py` (30), `test_decision_engine.py` (22 — the AI-layer seam, including `test_acceptance_is_not_authorisation`) |
| M5: v0.3.0 public methods still work; the 24 pre-existing tests pass **unmodified** | All 24 pre-existing test *functions* are unchanged (`test_adapters` 3, `test_executor` 1, `test_models` 8, `test_parsers` 6, `test_planner` 3, `test_scope` 3). One pre-existing file was extended: `tests/test_scope.py` gained 5 additive regression tests for the scope fix below (54 insertions, **0 deletions**). Full run: 377 passed |
| M6: correlation chain per execution and contract catalogue in every report | `tests/test_audit.py` (18 tests) proves the criterion directly: `test_chain_links_assessment_action_execution_evidence_verification_and_finding` and `test_every_finding_is_reachable_from_some_chain` assert the chain reaches `verification_ids` and `finding_ids`; `test_findings_from_other_executions_are_not_falsely_linked` guards against false linkage; `test_unattributed_execution_is_reported_honestly_not_backfilled` pins that a missing `action_id` is reported as missing. Plus `test_contract_pipeline.py::test_correlation_chain_links_action_to_execution_to_evidence` over a real subprocess, and a live CLI run whose `correlation_chains` entries all carry an `action_id` |
| M7: end-to-end loop proven against a real executable | `test_contract_pipeline.py` (25 tests) puts stub binaries on `PATH` and drives the production `IwDevAdapter`, `parsers/iw.py`, gateway and policy through real `subprocess` calls |
| Quality gates 1–6 | 377 tests pass in ~4s; `pyyaml` is still the only third-party runtime import (declared in `pyproject.toml`, grep-verified as the sole required import); no `shell=True` anywhere in `src/` (grep-verified: zero matches — the former defence-in-depth comment in `utils/validation.py` was removed with the no-op it described, and `tests/test_validation.py::test_no_module_in_the_package_invokes_a_shell` now enforces this by parsing the AST of every module rather than relying on a grep); all 119 modules under `wifi_framework` import cleanly with no tools installed (`pkgutil.walk_packages`, zero failures); a missing tool yields `unsupported`, never a fabricated success |
| Packaging: the documented install path works, and the tests do not secretly depend on the source tree | `pip install -e ".[dev]"` succeeds in a clean venv; the `wifi-assess` console script (declared in `project.scripts`) runs with `PYTHONPATH` unset and exits 0; the installed package reports `__version__ == 0.4.0`; and **all 264 tests pass with `PYTHONPATH` unset**, i.e. against the installed package rather than `src/`. Install artifacts (`*.egg-info/`, `.venv/`) are gitignored — `git status` stays clean after installing |
| Quality gate 5 (no dead code: every contract has a producer, a consumer and a test) | Verified by inspection script rather than by a single test: for each of the ten contracts, `PRODUCER` matches the ownership table, and every contract is referenced by at least one module under `core/` and at least one test file. Smallest case is `decision-proposal`/`planning-context` (1 core module each — `DecisionEngine.accept_proposal`/`planning_context`), now covered by `tests/test_decision_engine.py` |
